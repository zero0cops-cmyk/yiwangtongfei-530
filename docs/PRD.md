...
> 最后更新：2026-05-09
> 更新渠道：Claude Code（角色 B-1）
> 变更摘要：全局领域模型对齐——架次→任务实例、素材→原子素材；新增成果/工单日志实体；数据字典重构；工单通过event_ids关联上游事件；异常线索区改为事件级展示
...
| v2.3.0-SOP | 新增PENDING_ASSIGN直接关闭工单路径：状态机/数据字典/权限矩阵/FUNC规格/异常流同步更新 |
| v2.4.0-SOP | CLOSED拆分为两个终态：CLOSED（已完成·复核通过）与CANCELLED（已关闭·PENDING_ASSIGN直接关闭）；全模块同步 |
| v2.5.0-SOP | 全局领域模型对齐：架次→任务实例、素材→原子素材；§0.5业务流程Step5~7重构（实时链路+双渠道线索识别确认）；新增成果/工单日志数据字典；工单新增event_ids；异常线索区改为事件级展示；触发规格更新 |
...
| 任务实例 | 任务的一次实际执行单元（原称"架次"） | 任务、飞行记录、架次 |
| 静默生成 | 系统后台自动完成创建，不弹窗、不打断操作、不需用户确认 | 自动生成、后台生成 |
| 联动动作 | 状态流转时系统自动执行的写入或通知操作 | 副作用 |
| 线下核实与处置 | 委办局执行人员赴现场核实并实际解决问题，发生在系统外 | 复核、线上复核 |
| 原子素材 | 成果中的最小单位（单张照片／单段视频），是 GPS 确认、材料上传的操作粒度 | 素材 |
| 异常线索 | 上游事件聚合后的业务对象，是工单的来源；数据模型层面等价于事件 | 异常数据、问题数据 |
| 成果 | 任务实例回传的照片、视频等原子素材的集合容器 | — |
| 工单日志 | 工单生命周期中每一次操作的完整记录；工单与日志为 1:1 关系 | — |
...
| 模块 | 枚举值 | 标准显示文字 |
|---|---|---|
| 飞行任务实例 | `IN_PROGRESS` | 执行中 |
| 飞行任务实例 | `PENDING` | 待执行 |
| 飞行任务实例 | `COMPLETED` | 已完成 |
...
│ 6. 线索识别确认（双渠道）                              │
│    参与方：运营单位                                    │
│    渠道① 实时事件 → PC端确认事件 → 指派委办局管理员     │
│    渠道② 成果管理 → 飞行结束后核实全部素材与事件       │
│          → 指派委办局管理员                            │
│    输出：工单（PENDING_ASSIGN，已指定委办局管理员）     │
│    ├─ 确认有效 → 生成工单                             │
│    └─ 确认误报 → 直接关闭（填写关闭原因）              │
│        输出：工单（CANCELLED·已关闭·终态）             │
└──────────────────┬──────────────────┘
                   │
                   ▼
┌─────────────────────────────────────┐
...
需求申报(REQ) ──────────────────────────────────────> 任务配置
                                                         │
任务 ──1:N──> 任务实例 ──1:N──> 成果 ──1:N──> 原子素材 ──>（上游：事件）──> 工单 ──1:1──> 工单日志
                                                                              ↕（双向可查）
                                                                            任务实例
...
│ · 任务与任务实例（监控）、工单填报与复核等               │  │   申报栏对接需求申报；隐患栏对接隐患池               │
...
│
├── [MON] 任务监控与视频
│   ├── MON-001 [H5] 任务Tab·任务实例列表（执行中／待执行／已完成）
│   ├── MON-002 [H5] 任务实例详情页
│   ├── MON-003 [H5] 实时视频页（从执行中任务实例详情进入）
│   ├── MON-004 [H5] 视频回放页（从已完成任务实例详情进入）
...
- 上游事件复核完成（运营人员确认事件有效）
- 运营人员选择归属委办局（`dept_id`）
- `event_ids` 非空，至少1个有效事件
- 上游触发信号字段完整（`flight_id`、`station_addr`、`anomaly_media_ids`、`event_ids`）
- 事件必然关联原子素材（`anomaly_media_ids` 中间接关联）
...
| CLOSED | 已完成（复核通过后终态） | #52C41A |
| CANCELLED | 已关闭（PENDING_ASSIGN直接关闭后终态） | #8C8C8C |
...
| `dept_id` | Ref→Dept | 运营人员选择 | 必填；不可修改 | 归属委办局 |
| `flight_id` | Ref→Flight | 上游触发 | 必填；不可修改 | 来源任务实例 |
| `station_addr` | String | 任务配置继承 | 必填；不可修改 | 任务站点位置 |
| `anomaly_media_ids` | Ref[]→Media | 上游触发 | 至少1个；仅`label=CONFIRMED` | 关联确认异常原子素材 |
| `event_ids` | Ref[]→Event | 上游触发 | 至少1个；不可修改 | 关联上游事件（工单由事件驱动生成） |
...
| `closed_at` | Timestamp\|null | 流转写入 | 状态→CLOSED或CANCELLED时写入 | 关闭时间 |
| `closed_by` | Ref→User\|null | 流转写入 | 状态→CLOSED或CANCELLED时写入 | 关闭操作人ID（复核通过时=reviewed_by，直接关闭时=关闭操作人） |
| `close_reason` | String\|null | 关闭人填写 | 状态→CANCELLED时必填；状态→CLOSED时不填；max 200字 | 直接关闭原因（仅CANCELLED时非空） |
| `created_at` | Timestamp | 系统生成 | 不可修改 | 工单创建时间 |

##### 原子素材主体字段

> 原子素材是成果中的最小数据单元。工单通过 `event_ids → 事件 → 原子素材` 链路间接关联。

| 字段名 | 类型 | 来源 | 约束 | 说明 |
|---|---|---|---|---|
| `id` | String | 系统生成 | 唯一 | 原子素材编号 |
| `result_id` | Ref→Result | 上游关联 | 必填 | 归属成果ID |
| `anomaly_addr` | String | 坐标反解 | 必填 | 异常地点 |
| `gps_lat` | Float\|null | GPS自动写入 | 范围-90~90 | 该点确认纬度 |
| `gps_lng` | Float\|null | GPS自动写入 | 范围-180~180 | 该点确认经度 |
| `gps_confirmed_at` | Timestamp\|null | GPS自动写入 | 进入围栏即写入；一次性 | GPS确认时间 |
| `site_photos` | File[] | H5分项提交 | GPS确认后必填；至少1张 | 现场照片 |
| `site_note` | String\|null | H5分项提交 | 选填；max 500字 | 现场说明 |
| `ai_label` | String\|null | AI识别 | — | AI识别类型标签 |
| `confidence` | Float\|null | AI识别 | 0~1 | AI置信度 |
| `created_at` | Timestamp | 系统生成 | 不可修改 | 创建时间 |

##### 成果主体字段

| 字段名 | 类型 | 来源 | 约束 | 说明 |
|---|---|---|---|---|
| `id` | String | 系统生成 | 唯一 | 成果编号 |
| `task_instance_id` | Ref→TaskInstance | 上游关联 | 必填 | 归属任务实例 |
| `media_count` | Int | 系统统计 | 必填 | 包含原子素材数量 |
| `created_at` | Timestamp | 系统生成 | 不可修改 | 创建时间 |

##### 工单日志主体字段

> 工单与日志为 1:1 关系，记录工单生命周期的完整操作历史。

| 字段名 | 类型 | 来源 | 约束 | 说明 |
|---|---|---|---|---|
| `id` | String | 系统生成 | 唯一 | 日志编号 |
| `workorder_id` | Ref→WorkOrder | 系统关联 | 必填；1:1 | 归属工单 |
| `entries` | JSON[] | 操作写入 | — | 操作条目数组；每项含 `action` / `operator_id` / `timestamp` / `detail` |
...
```mermaid
stateDiagram-v2
    [*] --> PENDING_ASSIGN : 事件复核完成\n运营人员创建工单\n（选择委办局）

    PENDING_ASSIGN --> ASSIGNED : 运营人员／委办局管理员派发\n二次确认通过

    PENDING_ASSIGN --> CANCELLED : 管理员／运营人员直接关闭\n填写关闭原因 + 二次确认通过
...
```
  ...
  ⚠️  无退回路径（CLOSED / CANCELLED 终态不可逆）
  ⚠️  打回可循环，次数不限
  ⚠️  GPS围栏和材料上传以每条原子素材为单位
```
...
|---|---|---|---|---|
| 初始 | 事件复核完成，运营人员创建工单（选择委办局） | 待指派（PENDING_ASSIGN） | 运营人员(PC) | 写`created_at`、`dept_id` |
| 待指派（PENDING_ASSIGN） | 派发+二次确认 | 已指派（ASSIGNED） | 运营人员(PC)／委办局管理员(H5) | 写`assignee_id`、`dispatched_at`；推送H5通知 |
| 待指派（PENDING_ASSIGN） | 直接关闭 + 二次确认 | 已关闭（CANCELLED） | 运营人员(PC)／委办局管理员(H5) | 写`closed_by`、`closed_at`、`close_reason`（必填）；触发双端存档 |
...
│  [待复核(3)][已打回(1)][已完成(46)][已关闭(1)]              │  ← Tab导航（附带数量）
├──────────────────────────────────────────────────────────┤
│  [日期选择器]  [关键词输入]  [归属委办局下拉]               │
│                              [查询]  [重置]               │
├──────────────────────────────────────────────────────────┤
...
| 已关闭(N) | `COUNT WHERE status=CANCELLED AND DATE(created_at)=selected` |
...
| 7 | 关闭原因 | `close_reason` | 仅`status=CANCELLED`时展示；纯展示 |
...
显示条件同派发：`status == PENDING_ASSIGN` 且当前账号为运营人员（非观察员）

| 字段 | 控件 | 约束 |
|---|---|---|
| 关闭原因 | 文本输入 | 必填；max 200字；placeholder「请填写关闭原因」 |
| 确认关闭 | 次操作按钮 | 触发二次确认弹窗 |

**直接关闭弹窗：**
```
标题：「确认关闭工单？」
内容：关闭原因输入框（必填，max 200字）
操作：[取消]  [确认关闭]
确认后联动动作：
  1. status: PENDING_ASSIGN → CANCELLED
  2. 写入 closed_by、closed_at、close_reason（必填）
  3. 触发双端存档
```
...
- 标题：异常线索（确认异常`{COUNT}`个）
- 数据来源：`event_ids`（通过事件关联原子素材）
- 每个异常线索（事件）下展示其关联原子素材缩略图：图片 + AI识别类型标签 + 置信度
- 点击缩略图：大图模态框，支持左右切换
- 底部：「查看来源任务实例全部原子素材 →」跳转数据管理详情页

---

#### [WO-008] PC端详情页·处置记录时间线 [PC]

> 时间线数据源自工单日志（工单 1:1 工单日志），日志记录工单生命周期的完整操作历史。

**默认全部展开**

| 序号 | 节点标签 | 显示内容 | 视觉状态 |
|---|---|---|---|
| 1 | 飞行任务实例完成 | `flight_id` · 任务名称 · 完成时间 | 🟢 始终已完成 |
| 2 | 进入待识别确认 | 系统时间戳 | 🟢 始终已完成 |
| 3 | 开始识别确认 | 操作人姓名 · 开始时间 | 🟢 始终已完成 |
| 4 | 识别完成 | `review_type` · 确认异常N张 · 完成时间 | 🟢 始终已完成 |
| 5 | 工单生成 | `created_at` | 🟢 始终已完成 |
| — | 管理员直接关闭工单（→CANCELLED） | 操作人姓名 · `closed_at` · 关闭原因 | 🟢（仅直接关闭路径出现；若出现则节点6-10不出现） |
...
| 数据看板 | 工单管理看板（STAT-004·全量8状态） | ✓ | — |
| 数据看板 | 工单管理看板（STAT-004·待跟进/跟进中/待复核/已完成/已关闭） | — | ✓ |
| 功能入口 | 隐患上报 | ✓ | ✓ |
| 功能入口 | 需求申报 | ✓ | — |

##### 数据看板通用规则

- 今日／累计切换：全局联动刷新。其中「三组看板」（飞行任务 + 飞行成果 + 工单管理）适用于委办局管理员，三组同步切换、同步刷新；委办局执行人员首页仅展示工单管理一组看板（5 项：待跟进／跟进中／待复核／已完成／已关闭），今日／累计切换仅联动该组。
- 进入首页时自动刷新数据，不需手动下拉
- 看板卡片无跳转行为，纯数据展示
...
**异常线索区标题：**「确认异常 共{COUNT}个」；`COUNT` = `event_ids` 的数量。每个异常线索（事件）下展示其关联原子素材。

##### 底部操作区按权限显示规则

| 角色 | 工单状态 | 显示内容 |
|---|---|---|
| 委办局管理员 | PENDING_ASSIGN | [派发] [关闭工单] |
| 委办局管理员 | PENDING_REVIEW | [复核通过] [打回] |
| 委办局管理员 | 其他状态 | 无操作按钮 |
| 委办局执行人员 | 非常态 | [提交复核]（条件置灰） |
| 委办局执行人员 | CLOSED / CANCELLED | 无操作按钮 |

##### 提交复核可点击条件

仅当所有原子素材项同时满足以下两项时可点击；否则置灰：
- `gps_confirmed_at` 非空
...
| 指标 | 计算逻辑 |
|---|---|
| 已完成任务 | `COUNT WHERE status=COMPLETED`任务实例数 |
| 照片 | 归档照片总数 |
| 视频 | 归档视频总数 |
| 异常识别 | `COUNT WHERE label=CONFIRMED`原子素材总数 |
...
| 待复核 | `PENDING_REVIEW` | — |
| 已完成 | `CLOSED` | — |
| 已关闭 | `CANCELLED` | 执行人不会被指派到已关闭工单，该列始终为0 |
...
| E-024 | 对终态工单（CLOSED/CANCELLED）执行任何写操作 | H5/PC | 接口拒绝，Toast「工单已关闭」 | — |
| E-025 | 现场照片单张超出大小限制 | H5 | Toast「图片过大，请压缩后重试」；不上传 | — |
| E-026 | 视频流加载失败（MQTT断连） | H5 | 提示「视频加载失败，请检查网络」 | — |
| E-027 | 航迹页Cesium加载失败 | H5 | 提示「地图加载失败，请重试」 | — |
| E-028 | 直接关闭工单（→CANCELLED）时未填写关闭原因 | H5/PC | 前端校验拦截，关闭原因标红+提示「请填写关闭原因」；提交按钮不触发请求 | — |
...
*docs/PRD.md · v2.5.0-SOP · 变更20+全局领域模型对齐 · 成果/原子素材/事件/工单/日志链路*
